...
class HypOptReport:
    ...
    def print_common(self):
        ...
        print("TF    \t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (tf_score["accuracy"], tf_score["precision"], tf_score["recall"], tf_score["f1"]))
        print("TF-IDF\t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (tfidf_score["accuracy"], tfidf_score["precision"], tfidf_score["recall"], tfidf_score["f1"]))
        ...
        print("       \taccuracy\tprecision\trecall\t\tf1")
        print("english\t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (stop_score["accuracy"], stop_score["precision"], stop_score["recall"], stop_score["f1"]))
        print("none   \t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (
                nostop_score["accuracy"], nostop_score["precision"], nostop_score["recall"], nostop_score["f1"]))
        ...
        print("      \taccuracy\tprecision\trecall\t\tf1")
        print("(1,1) \t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (onegram_score["accuracy"], onegram_score["precision"], onegram_score["recall"], onegram_score["f1"]))
        print("(1,2) \t%0.3f   \t%0.3f   \t%0.3f   \t%0.3f" % (
                twogram_score["accuracy"], twogram_score["precision"], twogram_score["recall"], twogram_score["f1"]))
    ...
    def print_knn(self):
        ...
        for k in self.accuracy["clf__n_neighbors"].unique():
            k_score = {"accuracy": np.mean(self.accuracy[self.accuracy["clf__n_neighbors"] == k]["score"]),
                       ...
                       "f1": np.mean(self.recall[self.recall["clf__n_neighbors"] == k]["score"])
                      }
                      
            print("%-9s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % ("k = " + str(k), k_score["accuracy"], k_score["precision"], k_score["recall"], k_score["f1"]))
        ...
        for metric in self.accuracy["clf__metric"].unique():
            metric_score = {"accuracy": np.mean(self.accuracy[self.accuracy["clf__metric"] == k]["score"]),
                            ...
                            "f1": np.mean(self.recall[self.recall["clf__metric"] == k]["score"])
                            }

            print("%-10s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % (metric, k_score["accuracy"], k_score["precision"], k_score["recall"], k_score["f1"]))
    ...
    def print_nb(self):
        ...
        for dist in self.accuracy["clf"].unique():
            ...
            print("%-15s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % (
            dist, dist_score["accuracy"], dist_score["precision"], dist_score["recall"], dist_score["f1"]))
    ...
    def print_svm(self):
        ...
        for c in self.accuracy["clf__C"].unique():
            ...
            print("%-6s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % (
                str(c), c_score["accuracy"], c_score["precision"], c_score["recall"], c_score["f1"]))
    ...
    def print_dt(self):
        ...
        for criterion in self.accuracy["clf__criterion"].unique():
            ...
            print("%-7s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % (
                criterion, crit_score["accuracy"], crit_score["precision"], crit_score["recall"], crit_score["f1"]))
    ...
    def print_mlp(self):
        ...
        for activation in self.accuracy["clf__activation"].unique():
            ...
            print("%-8s\t%0.3f\t\t%0.3f\t\t%0.3f\t\t%0.3f" % (
                activation, act_score["accuracy"], act_score["precision"], act_score["recall"], act_score["f1"]))