...
def train_and_test(test,dist,cargs):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_reduced, prepare.getTarget(n_troll, n_nontroll), test_size=test, random_state=42,
                                                        shuffle=True)
    predicted = []
...
def optimize(test, verbose):
    ...
    if verbose:
        print("splitting data")
    X_train, X_test, y_train, y_test = train_test_split(tweets, prepare.getTarget(n_troll, n_nontroll), test_size=test, random_state=0)
